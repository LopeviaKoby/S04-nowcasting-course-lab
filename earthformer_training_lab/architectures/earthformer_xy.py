"""EarthFormer_xy architecture copied from the original CasCast repository.

Source in the local research repo:
  /home/henryjuarez/code_andre_sophy/codigo_github/Repo_CasCast/networks/earthformer_xy.py

The course training lab imports this file directly so the notebook can run
without depending on the full CasCast repository layout.
"""
...
def earthformer_base(in_len, out_len, height, width, **over):
    num_blocks = 2
    enc_attn_patterns = ["axial"] * num_blocks
    dec_self_attn_patterns = ["axial"] * num_blocks
    dec_cross_attn_patterns = ["cross_1x1"] * num_blocks
    ### new added 
    num_layers = over.get('initial_downsample_stack_conv_num_layers', 2)
    dim_list  = over.get('initial_downsample_stack_conv_dim_list', [32, 128])
    down_list = over.get('initial_downsample_stack_conv_downscale_list', [2, 2])
    nconv_list= over.get('initial_downsample_stack_conv_num_conv_list', [2, 2])
    ...
    model = CuboidTransformerModel(input_shape=[in_len, height, width, 1],
                                   ...
                                   initial_downsample_activation="leaky",
                                   # initial_downsample_type=="stack_conv"
                                   initial_downsample_stack_conv_num_layers=num_layers, #was 3
                                   initial_downsample_stack_conv_dim_list=dim_list, #[16, 64, 128]
                                   initial_downsample_stack_conv_downscale_list=down_list, #[3, 2, 2]
                                   initial_downsample_stack_conv_num_conv_list=nconv_list, #[2, 2, 2]
                                   # misc
                                   padding_type="zeros",
                                   z_init_method="zeros",
                                   checkpoint_level=0,
...
class EarthFormer_xy(nn.Module):
    def __init__(self, in_len, out_len, height, width, **kwargs) -> None:
        super().__init__()
        self.net = earthformer_base(in_len, out_len, height, width,**kwargs)
